<template>
  <div
    class="h-full py-8 md:px-0 px-4"
    :class="ismujer ? 'bg-mujer' : 'bg-not'"
  >
    <div class="w-full text-center">
      <h1 class="text-2xl md:text-4xl font-bold mb-10 text-white sm">
        {{ title }}
      </h1>
    </div>
    <div
      ...
      :class="ismujer ? 'text-av-morado-dark' : 'text-gray-600 '"
    >
      Aún no hay noticias
    </div>
    <div v-else>
      <div
        v-for="post in posts"
        :key="post.slug"
        class="md:w-8/12 bg-white rounded-md mx-auto p-8 shadow-md mb-4 md:mb-8"
      >
        <div class="w-full text-center">
